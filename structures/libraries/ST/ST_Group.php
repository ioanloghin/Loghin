...
class ST_Group extends ST_Project
{
	// identifiers
	...
	private $parent_id; // (integer) id-ul grupului parinte
	public  $brothers;	// (integer) numarul de frati
	// dimensions
	...
	{
		return (isset(parent::$groups[$this->level-1][$this->parent_id])) ? parent::$groups[$this->level-1][$this->parent_id] : FALSE;
	}
	// ========================================================================
	//
	//
	...
	//
	// ========================================================================
	public function insert_group($parent_id, &$instance)
	{
	...
	private function html_lines()
	{
		...
		$html_code .= '</div>'."\n\r";
		// pozitioneaza butonul pentru contract/expand
		switch($this->format)
		{
			default:
			case 1:// grupurile sunt pe orizontala
				$html_code .= '<div class="toogle"><a class="icon" style="margin-top:'.$this->blocks_max_h.'px;">&nbsp;</a></div>'."\n";
				break;
			case 2:// grupurile sunt pe verticala
				$html_code .= '<div class="toogle"><a class="icon" style="margin-top:'.($this->height/2-8).'px; margin-left:'.($this->blocks_max_w+8).'px;">&nbsp;</a></div>'."\n";
				break;
		}
		return $html_code;
	}
	...
	public function HTMLcode2()
	{
		...
		//krsort($this->inside_groups);
		$inside = NULL;// cod html
		$inside_d = false;// display - daca il afiseaza
		foreach($this->inside_groups as $obj_group)
		{
			if(in_array($obj_group->get('id'), parent::$session['groups']))
				$inside_d = true;
			/*else
			{
				$diff = $obj_group->full_height();
				$this->height -= $diff;
				// apeleaza parintii pentru a le scadea inaltimea
				$this_parent = $obj_group->get_parent();
				while($this_parent !== FALSE)
				{
					$obj_group->get_parent()->height -= $diff;
					$this_parent = $obj_group->get_parent();
				}
			}*/
			
			
			$inside .= $obj_group->HTMLcode2();
		}
		
		// atributele HTML ale grupului
		$html_code = '<div id="group-'.$this->id.'" class="group" style="height:'.$this->height.'px; margin-top:'.$this->top.'px; padding-left:'.($this->blocks_max_w+20+20).'px">'."\n\r";
		
		// adauga codul HTML al liniilor
		// vom folosi $this->p_top ca si inaltime pentru div-ul cu linii
		$html_code .= $this->html_rel();
		...
		// pozitioneaza butonul pentru contract/expand
		$html_code .= '<div class="toggle" style="left:'.($this->blocks_max_w+20).'px"><div class="line_ho"></div><a class="icon">&nbsp;</a></div>'."\n";
		
		// adauga codul HTML al grupurilor din interior
		$html_code .= '<div id="group-inside-'.$this->id.'" class="inside_groups" style="display:'.(($inside_d) ? 'block' : 'none').';height:'.$this->height.'px;">'.$inside.'</div>'."\n";
	...
	//
	// ========================================================================
	private function html_rel()
	{
		
		
		// lines
		$html_code = '<div class="lines">'."\n";
			$html_code .= '<div class="line_ho"></div>'."\n";
			if($this->brothers > 1)// mai mult de 1 child
			{
				if($this->index == 0)// first child
					$html_code .= '<div class="line_down"></div>'."\n";
				elseif(($this->index+1) == $this->brothers)// last child
					$html_code .= '<div class="line_up"></div>'."\n";
				else// middle child
					$html_code .= '<div class="line_both"></div>'."\n";
			}
		$html_code .= '</div>'."\n";
