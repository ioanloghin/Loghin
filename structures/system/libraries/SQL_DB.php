...
class SQL_DB
{
	...
	{
		self::$link_identifier = mysql_connect(MYSQL_SERVER, MYSQL_USER, MYSQL_PAROLA);
		mysql_select_db(BAZA_DATE, self::$link_identifier);
	...
	//
	// ========================================================================
	public function sql_left_join($table, $table2, $relation, $where = NULL, $order = NULL, $start = 0, $limit = 0)
	{
		if(!empty($where)) $where = "WHERE ($where)";
	    $LIMIT = (!$start && !$limit) ? '' : "LIMIT $start, $limit";
