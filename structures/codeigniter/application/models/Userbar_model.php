...
class Userbar_model extends CI_Model {
	...
	public function getAccounts($root_id) {
		...
		$this->db->select('roots_accounts.account_id as account_id, firstname, lastname');  
		$this->db->where('root_id',$root_id);
		$this->db->order_by('roots_accounts.account_id','asc');
		$this->db->join('accounts','accounts.account_id = roots_accounts.account_id');
		$query = $this->db->get('roots_accounts');
	...
	private function _getAdultsVisible($account_id) {
		...
		foreach($query->result_array() as $k => $account){
			...
			// Set Nickname
			$this->db->select('nickname_id, nickname'); 
			$this->db->from('accounts_nicknames'); 
			$this->db->where('account_id',$account_id);
			$query = $this->db->get();
	...
	private function _getInstitutionsVisible($account_id) {
		...
		// Parcurge accounturile
		$this->db->select('*'); 
		$this->db->from('institutions_admins ia'); 
		...
		$query = $this->db->get();
		foreach($query->result() as $k => $instit) {
