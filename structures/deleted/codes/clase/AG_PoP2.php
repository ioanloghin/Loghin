...
class AG_PoP2
{
	...
	//
	// ========================================================================
	public function __construct($my_id_tree, $my_id_family, $my_id_user, $my_rel = 0)
	{
		$this->my = array();
		$this->my['id_tree'] = 0;
		$this->my['id_family'] = 0;
		...
		if((!$my_id_family) && ($this->my['id_user'] > 0) && ($this->my['id_tree'] > 0))
		{
			...
			if(isset($temp[1][DBT_FAM_C1]))
				$this->my['id_family'] = intval($temp[1][DBT_FAM_C1]);
			unset($temp);
		}
		elseif($my_id_family)
		{
			if(is_int($my_id_family) && $my_id_family > 0)
				$this->my['id_family'] = $my_id_family;
		}
	...
	//
	// ========================================================================
	public function set_extern($extern_id_tree, $extern_id_family, $extern_id_user, $extern_rel)
	{
		if(is_numeric($extern_id_tree) && $extern_id_tree > 0)
			$this->extern['id_tree'] = intval($extern_id_tree);
		if(is_numeric($extern_id_family) && $extern_id_family > 0)
			$this->extern['id_family'] = intval($extern_id_family);
	...
	public function create($sets, $obj_refresh = FALSE)
	{
		...
		if(!$this->my['id_family'] && $this->my['id_tree'])
			$this->my['id_family'] = $this->new_family();
		
		// se adauga utilizatorul in familie
		/*if(!$this->linked['direction'] || in_array($this->linked['direction'], array('asc', 'desc')))
			$bool_insert = AG_Operation::family_user_add($this->my['id_family'], $this->my['id_user'], $this->my['id_rel']);
		else
	...
	public function import()
	{
		...
		if($this->my['id_user'])
		{
			...
			$bool_import = AG_Operation::family_user_change($this->my['id_family'], $this->my['id_user'], $this->extern['id_user'], $this->my['id_rel']);
			...
			foreach($family_user_reference as $local_id_family => $local_row)
				AG_Operation::family_user_change($local_id_family, $local_row['id_user'], $this->extern['id_user'], $local_row['id_rel']);
		}
		else
			$bool_import = AG_Operation::family_user_add($this->my['id_family'], $this->extern['id_user'], $this->my['id_rel']);
	...
	//
	// ========================================================================
	private function db_get_info($id_user)
	{
		$info = array();
	...
	public function import_local($id_partener, $children = array())
	{
		if(($id_partener) || (is_array($children) && count($children)))
		{
			...
			{
				$side = 2;
				$id_new_family = $this->my['id_family'];
			}
			else
			if($this->my['id_rel'] == 2)
			{
	...
	{
		if((is_int($new_user_id)) && ($new_user_id > 0) && ($this->from['id_family'] > 0))
		{
			// verificam daca este user default
			...
			//$bool_import1 = AG_Operation::family_user_change($this->from['id_family'], $this->my['id_user'], $new_user_id, $this->from['id_rel']);
			print "Family id = ".$this->from['id_family']." change user ".$this->my['id_user']." -> ".$new_user_id."<br />";
			// modificam toate familile care il au ca referinta pe user-ul inlocuit
			$family_user_reference = AG_Operation::family_user_reference($this->from['id_tree'], $this->my['id_user']);
			foreach($family_user_reference as $local_id_family => $local_row)
				//AG_Operation::family_user_change($local_id_family, $local_row['id_user'], $new_user_id, $local_row['id_rel']);
				print "[foreach] Family id = ".$local_id_family." change user ".$local_row['id_user']." -> ".$new_user_id."<br />";
		}
	}
	// ========================================================================
	//
	...
	{
		$bool = false;
		if($this->my['id_family'] > 0)
		{
			$is_parent = (in_array($this->my['id_rel'], array(1, 2)));
			$id_partener = 0;
			if($is_parent)
				$id_partener = AG_Operation::get_partener($this->my['id_family'], $this->my['id_user']);
			// daca este parinte iar partenerul nu este definit, se elimina familia
			if($is_parent && !$id_partener)
			{
				// eliminam familia
				if(!($bool = AG_Operation::family_delete($this->my['id_family'])))
					$this->errno = 301;
				// eliminam ex relatiile familiei
				AG_Operation::exrel_delete_all($this->my['id_family'], $this->my['id_family'], 0);
			}
				...
				switch($this->my['id_rel'])
				{
					...
					case 2:
						if($this->my['id_rel'] == 1)
							...
							$sets[DBT_FAM_C7] = 0;
						if(!($bool = AG_Operation::family_update($this->my['id_family'], $sets)))
							$this->errno = 302;
						break;
					case 3:
						if(!($bool = AG_Operation::family_children_delete($this->my['id_family'], $this->my['id_user'])))
							$this->errno = 303;
						break;
					default: $this->errno = 304; break;
				}
				// eliminam ex relatiile familiei
				AG_Operation::exrel_delete_all($this->my['id_family'], $this->my['id_family'], $this->my['id_user']);
			}
			...
			if(isset($remove_direction))
			{
				...
				foreach($member_ids as $local_id => $members)
				{
					...
					// eliminam ex relatiile familiei
					AG_Operation::exrel_delete_all($this->my['id_family'], $local_id, 0);
				}
			}
