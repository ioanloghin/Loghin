...
function AGchangeTree(next_tree_id, next_user_id, direction)
{
	AGmyPreview = false;
	if(AGmyPreview)
		window.location = ROOT+lang+'/preview/tree/graph/'+next_tree_id+'/'+next_user_id+'/'+direction;
	else
		window.location = ROOT+lang+'/tree/graph/'+next_tree_id+'/'+next_user_id+'/'+direction;
}

/* functia AGchangeProfile cu jquery */
function ag_member_change(direction, my_id_tree, my_id_family, my_id_user, my_id_rel)
{
	var htmlid_old_sufix = (AGmyIDRel == 3) ? 'c' : 'p';
	var htmlid_new_sufix = (my_id_rel == 3) ? 'c' : 'p';
	
	// obiecte html
	var doc_old = $('#AGuserBox_'+AGmyIDUser+htmlid_old_sufix);
	var doc_new = $('#AGuserBox_'+my_id_user+htmlid_new_sufix);
	var doc_ColoanaStanga = $('#AG_PageLeftBox');
	if(direction == 'asc')
	{
		var doc_icon_old = $('#AGUBIPA_'+AGmyIDUser+htmlid_old_sufix);
		var doc_icon_new = $('#AGUBIPA_'+my_id_user+htmlid_new_sufix);
	}
	else if(direction == 'desc')
	{
		var doc_icon_old = $('#AGUBIPD_'+AGmyIDUser+htmlid_old_sufix);
		var doc_icon_new = $('#AGUBIPD_'+my_id_user+htmlid_new_sufix);
	}
	
	// schimba profilul curent in variabilele globale js
	AGmyIDUser = my_id_user;
	AGmyIDRel = my_id_rel;

	if(doc_old && doc_new)
	{
		// deselecteaza profilul precedent
		doc_old.removeClass('ag_userbox_blue');
		
		// selecteaza profilul curent
		doc_new.addClass('ag_userbox_blue');
		
		// schimba iconitele pentru optiunile profilui
		if(direction == 'asc')
		{
			doc_icon_old.removeClass();
			doc_icon_old.addClass('ag_userbox_icon_profile_asc');
			
			doc_icon_new.removeClass();
			doc_icon_new.addClass('ag_userbox_icon_profile_ascOn');
		}
		else
		if(direction == 'desc')
		{
			doc_icon_old.removeClass();
			doc_icon_old.addClass('ag_userbox_icon_profile_desc');
			
			doc_icon_new.removeClass();
			doc_icon_new.addClass('ag_userbox_icon_profile_descOn');
		}
		...
		if(doc_ColoanaStanga)
		{
			...
			var sURL = ROOT+'views/tree/left_column.php?AGajax';
			sURL += '&lang='+lang;
			...
			{
				if(ajax.status == 200)
					doc_ColoanaStanga.html(ajax.responseText);
...
function AGchangeAddProfile(id_family, next_user_id, id_rel, linked_id_user, linked_direction)/* nu mai este nevoie */
{
	...
	if(doc_old && doc_new)
	{
		...
		if(doc_ColoanaStanga)
		{
			...
			var sURL = ROOT+'views/tree/left_column.php?AGmyIDUser=0';
			sURL += '&lang='+lang;
...
function AGmyView_session(action, direction, identif_family)
{
	...
	var sURL = ROOT+'models/AGmyView.php?AGmyIDTree='+AGmyIDTree+'&action='+action+'&direction='+direction+'&identif_family='+identif_family;
	ajax.open("GET", sURL, true);
	ajax.setRequestHeader("User-Agent", navigator.userAgent);
	ajax.send();
