...
class AG_FamilyBox
{
	...
	public $conectTo = array('family_id' => 0, 'id_user' => 0);
	...
	//
	// ========================================================================
	public function deleteMembersInFamilyObj($family_id, &$arrayFamilyObj)
	{
		if(isset($arrayFamilyObj[$family_id]))
		{
			$arrayFamilyObj[$family_id]->members_ref[1] = NULL;
			$arrayFamilyObj[$family_id]->members_ref[2] = NULL;
			$arrayFamilyObj[$family_id]->members_ref[3] = array();
		}
	}
	// ========================================================================
	//
	...
	public function calculateFamilyCoord(&$arrayFamilyObj)
	{
		switch($this->recognition['type'])
		{
			...
			case 2:
			case -2:
				$famConectML = $arrayFamilyObj[$this->conectTo['family_id']]->marginLeft;
				$famConectMT = $arrayFamilyObj[$this->conectTo['family_id']]->marginTop;
				$famConectW = $arrayFamilyObj[$this->conectTo['family_id']]->width;
				$famConectH = $arrayFamilyObj[$this->conectTo['family_id']]->height;
	...
	public function setAscDescMembers($familyArrayChildren, $familyArrayParents, $familyArrayIdentif, &$arrayFamilyObj)
	{
		...
		foreach($this->members_ref[3] as $key => $userbox)
		{
			...
			{
				$searchInParents = AG_Operation::searchInParents($userbox->member_id(), $familyArrayParents);
				$family_id = $searchInParents['family_id'];
				if($family_id)
				{
					$parent = $searchInParents['parent'];
					$identifier = (array_key_exists($family_id, $familyArrayIdentif)) ? $familyArrayIdentif[$family_id] : 0;
					$ascFamily = array('id' => $family_id, 'identifier' => $identifier, 'checked' => FALSE, 'readonly' => FALSE);
			...
			if($this->recognition['type'] == 2)
			{
				...
				//print '<pre>'.$userbox->info['id'].' -> '; var_export($family_id);
				//if($family_id && $family_id != $AGmyIDFamily) // TODO: nu stiu la ce ajuta asta
					//$this->deleteMembersInFamilyObj($family_id, $arrayFamilyObj);
				//
			...
			/*if($this->info['family_id'] == 2)
			{
				print '<pre>'.$key.' -> '; var_export($family_id);
			}*/
	...
	public function HTMLcode($display = 'block')
	{
		...
		$background_image = base_url().'models/AG_FamilyBoxLines.php';
		...
		{
			// parinte 1
			if($this->members_ref[1]->member_id() > 0)
			{
				$c1 = count($this->exRelatiiButtons[1]);
				$html_code .= '<div class="agFamilyBoxExRelButtons" style="width:'.AGFORMAT_T1_EX_W.'px; height:'.AGFORMAT_T1_EX_H.'px; margin-left:'.($this->members_ref[1]->marginLeft-AGFORMAT_T1_EX_W).'px; margin-top:'.$this->members_ref[1]->marginTop.'px; background-image:url('.base_url().'models/AG_FamilyBoxLines.php?familyType=1&amp;familyAlign=1&amp;familyWidth='.AGFORMAT_T1_EX_W.'&amp;familyHeight='.AGFORMAT_T1_EX_H.'&amp;exRelatii='.$c1.'&amp;option=2);">';
				for($n=5; $n>=1; $n--)
				{
			...
			// parinte 2
			if($this->members_ref[2]->member_id() > 0)
			{
				$c2 = count($this->exRelatiiButtons[2]);
				$html_code .= '<div class="agFamilyBoxExRelButtons" style="width:'.AGFORMAT_T1_EX_W.'px; height:'.AGFORMAT_T1_EX_H.'px; margin-left:'.($this->members_ref[2]->marginLeft+AGFORMAT_T1_UBW).'px; margin-top:'.$this->members_ref[2]->marginTop.'px; background-image:url('.base_url().'models/AG_FamilyBoxLines.php?familyType=1&amp;familyAlign=2&amp;familyWidth='.AGFORMAT_T1_EX_W.'&amp;familyHeight='.AGFORMAT_T1_EX_H.'&amp;exRelatii='.$c2.'&amp;option=2);">';
				for($n=5; $n>=1; $n--)
				{
					$marginT = ($n == 5) ? 'margin-top:7px;' : '';
	...
	private function getTagg()
	{
		...
		//$tagbox_value .= '<br />family_id: '.$this->info['family_id'];
		//$tagbox_value .= '<br />conectTo: family_id ['.$this->conectTo['family_id'].'],  id_user ['.$this->conectTo['id_user'].']';
