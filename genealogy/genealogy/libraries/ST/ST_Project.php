...
class ST_Project
{
	...
	// contained
	private $groups = array(); // (array(id => instance,)) referinte catre grupurile structurii
	//
	// ========================================================================
	public function __construct($projectID, $blockID, $deep, $direction = 1)
	{
		...
		if($this->blockID)
		{
			/* TODO: pfffff, asta nu merge
			   o sa elimin la afisare grupurile pe care nu dorim sa le afisam insa era mult mai bine
			   daca erau eliminate de aici (prelucram un volum de date mai mic)
		...
		foreach($inferiors as $sup_block => $inside_blocks)
		{
			if(count($inside_blocks))
			{
				...
				// initializare
				if(!array_key_exists($this_lvl, $this->groups)) $this->groups[$this_lvl] = array();
				
				// parcurge blocurile de pe nivel
				foreach($inside_blocks as $row_blockID)
				{
					// stabileste id-ul grupului
					$gr_id = $this_lvl.count($this->groups[$this_lvl]);// id-ul primului bloc va fi si id-ul grupului
					...
					// se creaza grupul
					$this->groups[$this_lvl][$gr_id] = new ST_Group($gr_id, $sup_block, $this_lvl, $this->direction);
					
					// se creaza si se adauga blocurile grupului
					$this->groups[$this_lvl][$gr_id]->insert_block($row_blockID, new ST_Block($row_blockID, $db_blocks[$row_blockID]['code'], $db_blocks[$row_blockID]['label'], $db_blocks[$row_blockID]['type']));
					
					// adauga acest grup la grupul parinte (daca are superior)
					if($sup_block > 0)
					{
						$parent = $block2group[$sup_block];// id-ul grupului parinte
						$this->groups[$this_lvl-1][$parent]->insert_group($parent, $this->groups[$this_lvl][$gr_id]/* referinta catre grupul curent */);
					}
		...
		foreach($this->groups as $gr_level => $gr_array)
		{
		...
		$levels = count($this->groups);// numarul de nivele
		foreach($this->groups as $gr_level => $gr_array)
		{
		...
		//
		//print '<pre>'; var_export($this->groups); print '</pre>';
	}
	// ========================================================================
	//
	//
	...
	{
		$html_code = '<div id="ST_Project" style="width:'.$this->width.'px; height:'.$this->height.'px;">';
		...
		foreach(current($this->groups) as $gr_id => $group)
		{
			$print = true;
