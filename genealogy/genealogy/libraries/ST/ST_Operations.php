...
class ST_Operations
{
	...
	public function ateco($lang)
	{
		...
		$db_blocks = Struct_model::get_blocks($this->projectID, 0, 0, 'code');
		...
		foreach($db_blocks as $code => $block)
		{
			...
			{
				$db_blocks[$code]['action'] = 3;
				// adauga codul in commands
				$this->commands[3][] = $code;
			}
		}
	...
	public function block_insert($block)
	{
		...
		$this_id = SQL_DB::sql_insert('struct_blocks', array(
	...
	public function block_update($block)
	{
		...
		// pregateste datele ce urmeaza a fi actualizate
		$sets = array();
		$sets['label'] = $block['label'];
		if(array_key_exists($block['sup'], $this->temp_ids))
			$sets['sup'] = $this->temp_ids[$block['sup']];
		
		// efectueaza actualizarea
		if(!SQL_DB::sql_update('struct_blocks', $sets, "`id` = '$block[id]'", 1))
		{
			// inregistreaza eroare si returneaza false
