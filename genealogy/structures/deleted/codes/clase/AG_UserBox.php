...
class AG_UserBox
{
	...
	public function HTMLcode($special = NULL)
	{
		...
		if(in_array($this->box_type, array(1, 2)))
		{
			...
			$html_code .= '<div class="agub_image"><img onclick="AGchangeProfile(\''.$AGchangeProfileDirection.'\', '.$AGmyIDTree.', '.$my_id_family.', '.$this->info[DBT_USER_INFO_C1].', '.$this->internalRelation['id'].');" style="float:left; background-color:#666;" src="'.ROOT.$img_src.'" width="50" height="50" alt="" /></div>';
			if($this->box_type == 1)
		...
		if($this->box_type == 3)
		{
			$html_code .= '<div class="agub_image"><img onclick="" src="'.ROOT.'design/imagini/icons/add-user.png" width="48" height="48" alt="" /></div>';
		}
        $html_code .= '</div>';
		return $html_code;
	}
