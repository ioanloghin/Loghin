{include file='header.tpl'}
    ...
    <table cellpadding="0" cellspacing="1" width="100%" class="gridTable">
		<tr>
			...
			<td class="gridHeader">{'group'|lang}</td>
        ...
        <tr class="gridTr">
			<td class="gridRow{if $item@index % 2 == 0}1{else}2{/if}">
            	<a class="grid" href="{$vir_cp_path}index.php?m=layouts&p=edit&id={$item@key}" title="{'edit'|lang}">{$item.name}</a>
           	</td>
			...
			<td class="gridActions" width="24" style="text-align:center">
				...
				<ul class="opmenu">
					<li><a href="{$vir_cp_path}index.php?m=layouts&p=edit&id={$item@key}">{'edit'|lang}</a></li>
					{if !$id}<li><a href="{$vir_cp_path}index.php?m=layouts&id={$item@key}">{'manage_sublayouts'|lang}</a></li>{/if}
					<li><a href="{$vir_cp_path}index.php?m=layouts&p=delete&id={$item@key}&page={$page}" onclick="return confirm('{'delete?'|lang}')">{'delete'|lang}</a></li>
				</ul>
			</td>
		</tr>
        {/foreach}
