...
{
	private $User_model;
	private $Myaccount_model;
	private $Userbar_model;
	private $session;
	
	public function __construct($User_model, $Myaccount_model, $Userbar_model) {
		//parent::__construct();
		
		$this->User_model = $User_model;
		$this->Myaccount_model = $Myaccount_model;
		$this->Userbar_model = $Userbar_model;
	...
	public function index() {
		...
		// FORCE LOGIN
		$account_id = isset($SESSION->conf['member_id']) ? $SESSION->conf['member_id'] : 0;
		$root_id = 1;
		
		$this->Myaccount_model->forceLogin($root_id,$account_id);
		...
		if($this->Myaccount_model->logged()) {
			...
			if($root_id) {
				...
				$nicknamesList = $this->Userbar_model->getNicknames($root_id);// currentAccountId
