    	...
    	<div id="container" class="min-width">
    		<div id="head-line" class="clear bg" style="background-image: linear-gradient(#E3E3E4, #FEFEFE);margin-bottom: 30px;">
    			<div class="left" style="font-size:13px;margin-top:16px;">
    				...
    				<a href="{$vir_path}image/">Image  &nbsp;|&nbsp; </a><br>
					...
					<li class="selected">
						...
						<ul>
						    {foreach $systemMenus AS $menu}
						    	{if $menu.type == 'left'}
						    <li class="home">
								<a href="#" class="lavel1"><span class="icon"> </span>{$menu.name}</a>
									{if $menu.items}
									<div class="sidebar-navigation">
								     <ul class="lavel2">
                    					{foreach $menu.items AS $_menu}
                    						<li>
                    						    <a data-id="{$_menu@key}" href="#">{$_menu}</a>
                    						    <ul class="lavel3">
                    						    {foreach $group_items AS $group_id=> $layout}
                    						        {if $layout.menu_id == $_menu@key}
                        						        <li>
                        						            <a data-id="{$layout@key}" href="#">{$layout.name}</a> 
                        						            <ul class="lavel4">
                            						            {foreach $group_level[$layout.id] AS $parent_id => $_layout}
                                                                    <li>
                                                                        <a>{$_layout.name}</a>                                                         						                 
                                                                    </li>    
                                                                {/foreach}   
