...
function editSite($id)
{
	...
	$fields = array('id' => $id, 'parent' => array('site' => array(), 'application' => array()), 'types' => array('site' => __('site'), 'application' => __('application')));
	$data = array();
...
function showSites($id, $page)
{
	...
	/* Result sites data */
	$result = $DB->query("SELECT `s`.`site_id`, `s`.`parent_id`, `s`.`type`, `s`.`status`, `d`.`name`, (SELECT GROUP_CONCAT(`language`) FROM `". DB_PREFIX ."sites_data` WHERE `site_id` = `s`.`site_id` AND `status` = '1') AS `languages`
