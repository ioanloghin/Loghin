$(function() {
	...
	$('#customize').bind('click', function() {
		if ($('#custBlock:visible').length == 0) {
			...
			$.customizeError = $('#custBlock .info').hasClass('error') ? true : false;
	...
	$('#custBlock input[type="checkbox"][name]').bind('click', function()
	{
		var options = {
			'm': 'customize',
			't': $(this).attr('name').replace('[]', ''),
			'id': $(this).val(),
			'e': $(this).attr('checked') ? 1 : 0
		};
		var loading = $('#custBlock').ajaxLoading('show');
		$.post($.conf.path, options, function(data)
		{
			...
			else if (data.success == true)
			{
				...
				{
					/* Update customize error variable */
					$.customizeError = false;
					...
					$.each(data.fav, function($type, $data)
					{
						...
						$.each($data, function(undefined, index)
						{
							/* Set id var */
							var id = index.id;
							/* Set checked on chackbox if need this */
							$('#custBlock input[type="checkbox"][name="'+ $type +'[]"][value="'+ id +'"]').attr('checked', true);
						...
						/* Check if all sites setted */
						if ($('#custBlock input[type="checkbox"][name="'+ $type +'[]"]:checked').length >= 6) {
							$('#custBlock input[type="checkbox"][name="'+ $type +'[]"]:not(:checked)').attr('disabled', true);
						}
						else if ($('#custBlock input[type="checkbox"][name="'+ $type +'[]"]:disabled'))
						{
							$('#custBlock input[type="checkbox"][name="'+ $type +'[]"]:disabled').attr('disabled', false);
					...
					/* Check if exist error */
					if (!$.customizeError && $('#custBlock .info').hasClass('error')) {
