					...
					$.post($.conf.path, options, function(data) {
						...
						else if (data.success == true)
						{
							...
							if (typeof data.fav != 'undefined' && !$.isEmptyObject(data.fav)) {
								...
								if ($('#custBlock input[type="checkbox"][name="items[]"]:checked').length < 12 && $('#custBlock .error').length < 1) {
                            ...
                            if (typeof data.items == 'object' && typeof data.id == 'string') {
                                ...
                                if (block.find('[name="id"]').val() == 0) {
                                    /*layout.find('dl:last').after('<dl class="clear">'+
				                        '<dt>'+
					                        '<label><input type="checkbox" name="layout[]" value="'+ data.id +'" /></label>'+
					                        '<span>Layout '+ data.order +'</span>'+
				                        '</dt>'+
				                        '<dd>'+ data.name +'</dd>'+
			                        '</dl>');
                                    layout.find('h4').remove();*/
                                    window.location.reload(true);
                                }
