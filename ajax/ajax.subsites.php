...
function subsites()
{
	...
	$result = $DB->query("SELECT `l`.`layout_id`, `d`.`name` FROM `". DB_PREFIX ."layouts` AS `l` LEFT JOIN `". DB_PREFIX ."layouts_data` AS `d` ON `d`.`layout_id` = `l`.`layout_id` AND `d`.`language` = '". SYS_LANG ."' WHERE `l`.`layout_id` IN ('". implode("', '", $id) ."') LIMIT 1");
