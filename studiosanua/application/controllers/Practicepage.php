...
class Practicepage extends MY_Restrictedzone {
	...
	public function layout($item_key = NULL, $form_lang = NULL)
	{
		...
		{
			$this->Layout->update_visible('practice', $item_key, $this->input->post('field_visible')=='1'?'1':'0');
			...
			switch($item_key)
			{
				...
				case 'leftmenu':
				{
					$this->LangModel->save_content('practice-leftmenu-title', $form_lang, $this->input->post('field_title'));
					break;
				}
			}
		...
		$data['field_visible'] = isset($_POST['field_visible'])
							...
							: $this->Layout->get_visible('practice', $item_key);
		switch($item_key)
		{
			case 'content':
			{
			...
			case 'leftmenu':
			{
				$data['field_title'] = isset($_POST['field_title'])
							...
							: $this->LangModel->get_content('practice-leftmenu-title', $form_lang);
				break;
			}
		}
		
		$data['visible_content'] = $this->Layout->get_visible('practice', 'content');
		$data['visible_leftmenu'] = $this->Layout->get_visible('practice', 'leftmenu');
		
		$this->load->view('admin/page-practice-layout', $data);
	}
}
